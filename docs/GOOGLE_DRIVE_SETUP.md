...
Google Driveを保存先とする個人プロジェクト管理機能（設定 > データ保存/読み込み → 保存先で「Google Drive」を選択）の外部設定手順です。
...
- `マイドライブ/EcorisMap/` フォルダに「1プロジェクト = 1 zipファイル（既存のローカル保存と同形式）」で保存
...
zipファイルはアプリの一覧に表示されない（アプリが作成したファイルのみ）。

## 1. Google Cloud Console設定

Firebaseプロジェクト（`ecorismap`）はGCPプロジェクトなので、それをそのまま使う。
...
`keys/googleDriveOAuth.ts` を作成する:
...
1. `yarn web` → 設定 > データ保存/読み込み → 保存先「Google Drive」→ 接続 → 保存 → Drive Web UIで `EcorisMap/` にファイル確認
...
- 機能の有効/無効はビルドフラグではなく`googleDriveOAuth`のクライアントID設定有無で決まる（空なら「利用不可」として無効化）

...
export class FetchSource implements Source {
  ...
  async getBytes(offset: number, length: number, signal?: AbortSignal): Promise<RangeResponse> {
    ...
    let resp = await RNFetchBlob.fetch('GET', this.url, {
    ...
    if (resp.info().status === 416 && offset === 0) {
      // some HTTP servers don't accept ranges beyond the end of the resource.
      // Retry with the exact length
      const content_range = resp.info().headers['content-range'];
      if (!content_range || !content_range.startsWith('bytes */')) {
        throw Error('Missing content-length on 416 response');
      }
      const actual_length = +content_range.substr(8);
      resp = await RNFetchBlob.fetch('GET', this.url, {
        Range: 'bytes=0-' + (actual_length - 1),
      });
    }

    if (resp.info().status >= 300) {
      throw Error('Bad response code: ' + resp.info().status);
    }

    const content_length = resp.info().headers['content-length'];
    ...
    if (resp.info().status === 200 && (!content_length || +content_length > length)) {
    ...
    return {
      data: buffer.buffer,
      etag: resp.info().headers.etag || undefined,
      cacheControl: resp.info().headers['cache-control'] || undefined,
      expires: resp.info().headers.expires || undefined,
    };
  }
}
...
async function getHeaderAndRoot(
  ...
  if (prefetch) {
    ...
    const dirKey =
      source.getKey() + '|' + (header.etag || '') + '|' + header.rootDirectoryOffset + '|' + header.rootDirectoryLength;
...
export class ResolvedValueCache {
  ...
  constructor(maxCacheEntries = 100, prefetch = true, decompress: DecompressFunc = defaultDecompress) {

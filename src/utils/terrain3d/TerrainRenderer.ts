...
/**
 * 遮蔽判定用: 視線方向の距離をRGB 24bitへ詰めて書き出す。
 *
 * ドットはキャンバスの外側（Reactのビュー）に重ねるので深度テストが効かない。
 * GPUが実際に描いた地形までの距離を読み戻し、点までの距離と比べて判定する
 * （JS側に地形の複製を持たないので、両者が食い違いようがない）。
 * NDC深度ではなく視線距離を書くのは、リング毎の深度レンジ分割に左右されず、
 * 許容差をメートルで考えられるため
 */
@fragment

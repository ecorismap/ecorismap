...
let currentState: TileMapType[] = initialMaps;
...
const executeThunkAndGetPayload = (thunkFn: (dispatch: jest.Mock, getState: () => { tileMaps: TileMapType[] }) => void): TileMapType[] => {
  const innerDispatch = jest.fn();
  const getState = () => ({ tileMaps: currentState });
...
// Thunk関数を実行して内部のdispatch呼び出し全体を取得するヘルパー
const executeThunkAndGetAction = (thunkFn: (dispatch: jest.Mock, getState: () => { tileMaps: TileMapType[] }) => void): { type: string; payload: unknown } | null => {
  const innerDispatch = jest.fn();
  const getState = () => ({ tileMaps: currentState });
  thunkFn(innerDispatch, getState);
  if (innerDispatch.mock.calls.length > 0) {
...
describe('useMaps', () => {
  ...
  // Thunkを使用することでstale closure問題を防いでいることを確認します。
  ...
  // stale closure対策はThunkを使用することで実装されている
  ...
  test('updateMapOrder: 地図の順序を変更できる', () => {
    ...
    expect(mockDispatch).toHaveBeenCalledTimes(1);
    const thunkFn = mockDispatch.mock.calls[0][0];
    const dispatchPayload = executeThunkAndGetPayload(thunkFn);
  ...
  test('saveMap: 既存マップを更新する場合、updateTileMapActionがdispatchされる', () => {
    ...
    // saveMapはThunkなのでThunk関数が渡される
    const thunkFn = mockDispatch.mock.calls[0][0];
    const action = executeThunkAndGetAction(thunkFn);

    // updateTileMapActionがdispatchされることを確認
    expect(action?.type).toBe('tileMaps/updateTileMapAction');
    expect(action?.payload).toEqual(updatedM1);
  });
  ...
  test('saveMap: 新規マップを追加する場合、addTileMapActionがdispatchされる', () => {
    ...
    expect(action?.type).toBe('tileMaps/addTileMapAction');
    expect(action?.payload).toEqual(newMap);
  });
  ...
  test('saveMap: 古いコールバック参照でも最新のmapsを参照して正しく判定する（stale closure対策）', () => {
    ...
    expect(mockDispatch).toHaveBeenCalledTimes(1);
    const thunkFn1 = mockDispatch.mock.calls[0][0];
    const action1 = executeThunkAndGetAction(thunkFn1);
    expect(action1?.type).toBe('tileMaps/addTileMapAction');

    // 新規マップが追加された後のstateをシミュレート
    const updatedMaps = [...initialMaps, newMap];
    mockSelector.mockReturnValue(updatedMaps);
    currentState = updatedMaps;
    rerender();
    ...
    expect(mockDispatch).toHaveBeenCalledTimes(2);
    const thunkFn2 = mockDispatch.mock.calls[1][0];
    const action2 = executeThunkAndGetAction(thunkFn2);
    expect(action2?.type).toBe('tileMaps/updateTileMapAction');
  });

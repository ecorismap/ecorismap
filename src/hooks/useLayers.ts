...
export const useLayers = (): UseLayersReturnType => {
  ...
  const changeLayerOrder = useCallback(
    (index: number, direction: 'up' | 'down') => {
      dispatch((thunkDispatch, getState) => {
        const newLayers = cloneDeep((getState() as RootState).layers);
        if (direction === 'up') {
          if (index === 0) return;
          ...
          if (currentLayer.type === 'LAYERGROUP') {
            ...
            const childLayers = newLayers.filter((layer: LayerType) => layer.groupId === currentLayer.id);
            ...
            if (previousLayer.groupId === undefined) {
              ...
              // 上のレイヤーがグループに入っている場合
              const groupParentIndex = newLayers.findIndex((layer: LayerType) => layer.id === previousLayer.groupId);
            ...
            if (previousLayer.type === 'LAYERGROUP' && currentLayer.groupId !== previousLayer.id) {
              ...
              const groupParentIndex = newLayers.findIndex((layer: LayerType) => layer.id === currentLayer.groupId);
              if (groupParentIndex !== -1) {
          ...
          if (currentLayer.type === 'LAYERGROUP') {
            ...
            if (nextItem.type === 'LAYERGROUP') {
              ...
              const nextGroupChildren = newLayers.filter((layer: LayerType) => layer.groupId === nextItem.id);
            ...
            if (nextLayer.type === 'LAYERGROUP' && currentLayer.groupId !== nextLayer.id) {
              ...
              const parent = newLayers.find((l: LayerType) => l.id === nextLayer.groupId);
  ...
  const onDragBegin = useCallback(
    (layer: LayerType) => {
      dispatch((thunkDispatch, getState) => {
        ...
        // getState()から最新のlayersを取得（stale closure対策）
        const currentLayers = (getState() as RootState).layers;
        const index = currentLayers.findIndex(({ id }: LayerType) => id === layer.id);
        const item = currentLayers[index];
        if (item.type === 'LAYERGROUP') {
          const newLayers = currentLayers.map((l: LayerType) => {
  ...
  const updateLayersOrder = useCallback(
    (data: LayerType[], from: number, to: number) => {
      ...
      dispatch((thunkDispatch, getState) => {
        ...
        const fromIndex = currentLayers.findIndex(({ id }: LayerType) => id === data[from].id);
        const toIndex =
          to > data.length - 1 ? currentLayers.length : currentLayers.findIndex(({ id }: LayerType) => id === data[to].id);
        const newLayers = cloneDeep(currentLayers);
        ...
        if (draggedLayer.type === 'LAYERGROUP') {
          ...
          if (targetLayer && targetLayer.expanded && (targetLayer.type === 'LAYERGROUP' || targetLayer.groupId)) {
            ...
            const groupTileMaps = newLayers.filter(
              (layer: LayerType) => layer.id === draggedLayer.id || layer.groupId === draggedLayer.id
            );

...
import { checkCoordsInput, checkFieldInput, updateRecordCoords, updateRecordPhoto } from '../utils/Data';
...
export const useDataEdit = (
  ...
  const downloadPhoto = useCallback(async () => {
    if (!hasOpened(projectId)) {
      return { isOK: false, message: t('hooks.message.openProject') };
    }

    const { url, key, name, fieldName, index } = selectedPhoto;

    if (url === null || key === null) {
      return { isOK: false, message: t('hooks.message.unkownURL') };
    }
  ...
  }, [projectId, selectedPhoto, targetLayer.id, dataUser.uid, targetRecord, setIsEditingRecord]);
